class HomeController < ApplicationController
  ...
  def request_form
    ...
    Request.transaction do
      ...
      @user = User.find_or_create_by(
        first_name: params[:request][:first_name],
        last_name: params[:request][:last_name],
        patronymic: params[:request][:patronymic],
        email: params[:request][:email],
        phone: params[:request][:phone]
      )
      @request.user = @user
  ...
  def get_institutions
    @institutions = Institution.select('DISTINCT(name), *').where(city_id: params[:city],
                                                                  institution_type_id: params[:institution_type]).order('institutions.name ASC')
